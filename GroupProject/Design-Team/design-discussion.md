...
| John Youngblood | The most notable commonality between my design and the team's design is the inclusion of SavedWordScramble, which is a stereotyped entity class of WordScramble. This design decision was made to facilitate the requirements for how a WordScramble is created and subsequently saved to the EWS, becoming uneditable. Other commonalities include the Player class and the Scramble interface. | One differences between my design and the team's design is the changing of the Statistics classes from 3 separate entities to a single interface. While initially thought of as a pro, we decided to leave it out since the necessary statistics outlined in the requirements are easily computed from the data returned by EWS's getPlayersAndSolvedSrambles() and getScrambleAndCreators() operations. There is also the Game and ProgressTracker classes in the team design that help better represent how a Player goes about solving a WordScramble, and how a Player can back out of solving but have their progress saved. |
